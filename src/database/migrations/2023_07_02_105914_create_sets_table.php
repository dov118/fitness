...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('sets', function (Blueprint $table) {
            ...
            $table->integer('index')->nullable(false)->default(1)
                ->comment('Index de la série pour l\'exercice');

            $table->integer('target')->nullable(false)->default(1)
                ->comment('Nombre de répétitions cible');
            $table->integer('limit')->nullable(false)->default(1)
                ->comment('Nombre de répétitions limite');
            $table->integer('min')->nullable(false)->default(1)
                ->comment('Nombre de répétitions minimum');
            $table->integer('max')->nullable(false)->default(1)
                ->comment('Nombre de répétitions maximum');

            $table->integer('left')->nullable(false)->default(1)
                ->comment('Nombre de répétitions à gauche');
            $table->boolean('left_failure')->nullable(false)->default(true)
                ->comment('Côté gauche à l\'échec');
            $table->integer('right')->nullable(false)->default(1)
                ->comment('Nombre de répétitions à droite');
            $table->boolean('right_failure')->nullable(false)->default(true)
            ...
            $table->dateTime('stop')->nullable(false)->default(now()->add('minutes', 2))
                ->comment('Fin de l\'étape');
            $table->float('duration')->nullable(false)->default(27)
            ...
            $table->boolean('rest')->nullable(false)->default(true)
