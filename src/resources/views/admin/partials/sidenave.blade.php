...
<nav class="menu" aria-labelledby="menu-heading">
    ...
    @can('viewAny', Exercise::class)<a class="menu-item" @if('admin.exercise.index' === Route::current()->getName()) aria-current="page" @endif href="{{ route('admin.exercise.index') }}"><span class="Counter">{{ count($exercises ?? []) ?: '??' }}</span>  All</a>@endcan
</nav>
